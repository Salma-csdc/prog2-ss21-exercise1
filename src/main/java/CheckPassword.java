...
public class CheckPassword {
    ...
    public boolean checkPassword(String kennwort) {
        ...
        boolean isSameValue= checkselbeZahl(kennwort);
        return isCorrectLength && isCorrectCase && isCorrectNumbers && isCorrectSpecialCharacter && isAvailable3Values && isSameValue;
    }
    ...
    public static boolean checkselbeZahl(String kennwort){
        ...
        if(checkZahlen(kennwort) == true){
            ...
            for(int i = 0; i < kennwort.length(); i++){
                ...
                if(i < kennwort.length()-3){    firstvalue = kennwort.charAt(i);
                                                secondvalue = kennwort.charAt(i+1);
                                                thirdvalue = kennwort.charAt(i+2);
                                                lastvalue = kennwort.charAt(i+3); }

                else if(i == kennwort.length()-3){ secondvalue = kennwort.charAt(i); }
                else if(i == kennwort.length()-2){ thirdvalue = kennwort.charAt(i); }
                else if(i == kennwort.length()-1){ lastvalue = kennwort.charAt(i); }
